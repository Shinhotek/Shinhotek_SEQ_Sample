...
> [!IMPORTANT]  
> 본 매뉴얼에서 제공하는 Lumosity Software Interface (XML / .NET Framework 기반 인터페이스) 는 사용자가 직접 프로그래밍을 통해 제어, 설정 및 자동 동작 시나리오를 구성할 수 있도록 제공됩니다.
>
> 이에 따라, 명령어의 실행 순서, 설정 값의 조합, 통신 타이밍 또는 외부 프로그램의 구현 방식에 따라 아래와 같은 문제가 발생할 수 있습니다.
> - 측정 결과의 비정상 동작 또는 오류 발생
> - 장치와의 통신 연결 불안정 또는 연결 해제
> - Lumosity Software 의 동작 이상 또는 일시적인 오동작
>
> 이러한 문제는 인터페이스의 프로그래밍 특성상 사용자의 구현 방식에 따라 발생할 수 있으며, 본 매뉴얼은 해당 인터페이스 사용에 대한 기능 설명을 목적으로 하며, 사용자 구현에 의해 발생하는 문제에 대해서는 책임을 지지 않습니다.
> 안정적인 사용을 위해서는 본 매뉴얼에 명시된 권장 사용 방법과 명령어 흐름을 준수할 것을 권장합니다.
